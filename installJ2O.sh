...
# === INSTALL/UPGRADE PODMAN 4+ IF NEEDED ===
echo "Checking if podman >= 4.1 is installed..."

# Helper: check whether installed podman meets minimum version 4.1
podman_version_ok() {
    local min_major=4 min_minor=1
    local ver
...
# Install podman 4+; uses Kubic repo on Ubuntu 22.04, default repos on 24.04
install_podman4() {
    ...
    case "$ubuntu_version" in
        22.04)
            echo "Ubuntu 22.04 detected. Adding Kubic repository for podman 4..."
            local key_url="https://download.opensuse.org/repositories/devel:/kubic:/libcontainers:/unstable/xUbuntu_${ubuntu_version}/Release.key"
            local sources_url="https://download.opensuse.org/repositories/devel:/kubic:/libcontainers:/unstable/xUbuntu_${ubuntu_version}"
            ...
            ;;
        24.04)
            echo "Ubuntu 24.04 detected. Installing podman from default repositories..."
            apt update
            apt install -y podman
            ;;
        *)
            echo "ERROR: Unsupported Ubuntu version: $ubuntu_version. This script supports Ubuntu 22.04 and 24.04."
            exit 1
            ;;
    esac
}

if command -v podman &> /dev/null && podman_version_ok; then
    echo "Podman $(podman --version | awk '{print $3}') is installed and meets the minimum version requirement (>= 4.1)."
else
    if command -v podman &> /dev/null; then
        echo "Installed podman version ($(podman --version | awk '{print $3}')) is too old. Minimum required: 4.1"
    else
        echo "Podman is not installed."
    fi
    echo "Installing podman 4+..."
    install_podman4
